...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <!-- Tambahkan Input Field untuk Rating -->
            <div class="form-group">
                <label for="rating">{{ trans('cruds.product.fields.rating') }}</label>
                <input class="form-control {{ $errors->has('rating') ? 'is-invalid' : '' }}" type="number" name="rating" id="rating" value="{{ old('rating', $product->rating) }}" step="0.1" min="0" max="5">
            ...
            <!-- Tambahkan Input Field untuk Review -->
            <div class="form-group">
                <label for="review">{{ trans('cruds.product.fields.review') }}</label>
                <textarea class="form-control {{ $errors->has('review') ? 'is-invalid' : '' }}" name="review" id="review">{{ old('review', $product->review) }}</textarea>
                @if($errors->has('review'))
                    <div class="invalid-feedback">
                        {{ $errors->first('review') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.review_helper') }}</span>
            </div>
